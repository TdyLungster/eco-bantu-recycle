        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center py-3">
            ...
            <div className="hidden lg:flex items-center space-x-8">
              {navItems.map(item => <button key={item.name} onClick={() => handleServiceClick(item.href)} className={`group flex items-center space-x-1 px-3 py-2 rounded-lg transition-all ${isActive(item.href) ? 'text-eco-primary bg-eco-primary/10 font-semibold' : 'text-gray-700 hover:text-eco-primary hover:bg-eco-primary/5'}`}>
                  <item.icon className="w-4 h-4" />
                  <span className="font-medium">{item.name}</span>
                  <div className={`h-0.5 w-0 bg-eco-primary transition-all group-hover:w-full ${isActive(item.href) ? 'w-full' : ''}`}></div>
                </button>)}
              
              {/* Auth Buttons */}
              <div className="flex items-center space-x-3">
                {isLoggedIn ? <div className="flex items-center space-x-3">
                    <motion.button className="flex items-center space-x-2 text-gray-700 hover:text-eco-primary transition-colors" whileHover={{
                  scale: 1.05
                }}>
                  ...
                  scale: 1.05
                    ...
                    <motion.button onClick={() => handleAuthClick('login')} className="flex items-center space-x-2 text-gray-700 hover:text-eco-primary transition-colors" whileHover={{
                  scale: 1.05
                }}>
                      <LogIn className="w-4 h-4" />
                      <span>Login</span>
                    </motion.button>
                    <motion.button onClick={() => handleAuthClick('signup')} className="flex items-center space-x-2 bg-eco-primary text-white px-4 py-2 rounded-lg font-medium hover:bg-eco-primary/90 transition-colors" whileHover={{
                  scale: 1.05
                }} whileTap={{
                  scale: 0.95
                }}>
              ...
              <motion.button onClick={() => handleServiceClick('#contact')} className="bg-eco-gradient text-white px-6 py-2.5 rounded-full font-semibold shadow-lg hover:shadow-xl transition-all" whileHover={{
              scale: 1.05
            }} whileTap={{
              scale: 0.95
            }}>
            ...
            {/* Mobile Menu Button */}
            <button className="lg:hidden text-gray-700 hover:text-eco-primary transition-colors" onClick={() => setIsOpen(!isOpen)}>
          ...
          height: 0
              ...
              setIsOpen(false);
            }} className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all w-full text-left ${isActive(item.href) ? 'text-eco-primary bg-eco-primary/10 font-semibold' : 'text-gray-700 hover:text-eco-primary hover:bg-eco-primary/5'}`}>
